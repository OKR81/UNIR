...
library(ggplot2)     # Para grÃ¡ficos 2D
library(plotly)      # Para grÃ¡ficos 3D
library(readr)       # Para leer archivos CSV
library(stats)       # Para realizar PCA
library(factoextra)  # Para eigenvalue y visualizaciÃ³n de PCA
...
any(is.na(datos_limpios)) # DeberÃ­a retornar FALSE
...
# Normalizar todos los datos numÃ©ricos
...
print(eigenvalues) # Verificar que con dos dimensiones se explica mÃ¡s del 60-70% de la varianza acumulada

# ComprobaciÃ³n visual de la varianza explicada por cada componente
fviz_eig(pca, addlabels = TRUE, ylim = c(0, 20))

# Visualizar las variables mÃ¡s destacadas segÃºn sus contribuciones en los ejes PC1 y PC2
fviz_contrib(pca, choice = "var", axes = 1, top = 10) 
fviz_contrib(pca, choice = "var", axes = 2, top = 10)


## Crear grÃ¡ficos de los componentes principales

# GrÃ¡fico para visualizar las dos primeras componentes principales
fviz_pca_ind(pca, geom.ind = "point", 
             col.ind = "blue", 
             axes = c(1, 2), 
             pointsize = 1.5) 

# Visualizar el grÃ¡fico de correlaciÃ³n de las variable 
...
# Seleccionar variables sociodemogrÃ¡ficas continuas
socio_continuas <- select(datos_limpios_norm, altura, peso, IMC, edad, METs_h_semana)

# Ver las variables sociodemogrÃ¡ficas continuas
View(socio_continuas)

# Combinar variables sociodemogrÃ¡ficas con los terciles de PC1
...
# Combinar variables sociodemogrÃ¡ficas con los terciles de PC2
...
# Preparar el dataframe (datos) para regresiÃ³n logÃ­stica
...
# Dividir en terciles variables sociodemográficas
...
# Ver los resultados del modelo logÃ­stico
summary(modelo_logistico)
